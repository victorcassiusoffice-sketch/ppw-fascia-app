...
function BodyMap({ session, setSession }) {
  ...
  const bodyImg = `${baseUrl}assets/body_${view}.png`;
      ...
      <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start">
        <div className="mx-auto w-full" style={{ maxWidth: 400 }}>
          ...
          <div className="relative">
            <img src={bodyImg} alt={`body map ${view}`} className="block w-full select-none pointer-events-none" draggable="false" />
            ...
            <svg viewBox="0 0 600 1200" className="absolute inset-0 w-full h-full">
              {hotspots.map(h => {
                const isSelected = !!selected[h.code];
                const cls = 'hotspot' + (isSelected ? ' selected' : '');
                const pain = selected[h.code];
                const common = { className: cls, onClick: () => toggle(h.code) };
                return (
                  <g key={h.code}>
                    {h.shape === 'ellipse'
                      ? <ellipse {...common} cx={h.cx} cy={h.cy} rx={h.rx} ry={h.ry} />
                      : <rect    {...common} x={h.x} y={h.y} width={h.w} height={h.h} rx="10" />
                    }
                    {isSelected && (
                      <text x={h.cx || h.x + h.w / 2} y={(h.cy || h.y + h.h / 2) + 5} textAnchor="middle" fill="#0a1628" fontSize="22" fontWeight="800" fontFamily="Syne, sans-serif" style={{ pointerEvents: 'none' }}>{pain}</text>
                    )}
                  </g>
                );
              })}
