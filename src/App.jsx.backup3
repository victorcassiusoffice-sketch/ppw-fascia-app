...
function BodyMap({ session, setSession }) {
  ...
  // Sub-Chat 5: anatomical PNGs from Sub-Chat 3 live in body_zones/ now.
  // Three-stage fallback: new path → legacy path → solid navy rect (graceful degrade).
  const primaryBodyImg  = `${baseUrl}assets/body_zones/body_${view}.png`;
