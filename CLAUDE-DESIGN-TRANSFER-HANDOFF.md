...
- [x] **Vic feature batch (2026-07-06 evening):** AUTO tickbox per stack (replaces row done-tick; slot engine opens it at its time) · hold-&-drag reorder with times-stay-with-positions · no-fixed-time stacks (Repeat sheet toggle → "Anytime" Next-Up queue at top) · Crimson soft theme from Vic's red logo (`public/assets/ppw-logo-crimson.png`) · Glass rebuilt: prototype scenes DELETED, Vic's 17 backgrounds (`public/assets/backgrounds/`) + own-photo + Frosted/Clear + Text Light/Dark toggles · Routines (premium): Create Routine builder in Library, saved unlimited (`ppw5.routines`), apply-to-any-day from Calendar (staggered 09:00+30min, repeat once).
- [ ] **Remaining** (needs external deps or lower value): Stack assistant orb CHAT panel (needs a real AI endpoint) · real lock-screen `.ics`/push wiring · doc upload (Add/Library) · blue-light filter + quiet hours · Connected apps + Course logins in Settings · Easy set-up flow · first-use tips · routine items: per-item time editing inside the builder
